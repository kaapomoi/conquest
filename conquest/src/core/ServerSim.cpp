...
ServerSim::ServerSim(k2d::vi2d map_size, int num_colors, int num_maps) :
    ...
    num_maps(num_maps)
{
    ...
    //maps[0] = "";
...
int ServerSim::StartGame(int map_index)
{
    ...
    create_game(players.size(), map_index);
...
void ServerSim::create_game(int num_players, int map_index)
{
    ...
    for (uint8_t y = 0; y < map_size.y; y++)
    {
        ...
        for (uint8_t x = 0; x < map_size.x; x++)
        {
            ...
            tilemap[y][x] = { (uint8_t) (maps[map_index][y * map_size.x + x] - '0'), 9 };
...
void ServerSim::CreateNewMaps()
{
    ...
    for (size_t i = 0; i < num_maps; i++)
    {
        ...
        {
            map.append(std::to_string(num_gen(rand_engine)));
        }
        maps.push_back(map);
    }
}

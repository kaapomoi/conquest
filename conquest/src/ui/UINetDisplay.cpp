...
void UINetDisplay::SetNeuralNetPtr(NeuralNet* nn)
{
	...
	float gap_width = size.x / (layers.size()-1) - node_width * layers.size();
	...
	for (size_t i = 0; i < layers.size(); i++)
	{
		...
		start_y = position.y - size.y * 0.5f;//(layers[0].size() - layers[i].size()) * (node_height + gap_height) / 4.0f;
...
void UINetDisplay::UpdatePositions()
{
	...
	if (net)
	{
		...
		for (size_t i = 0; i < layers.size(); i++)
		{
			for (size_t j = 0; j < layers[i].size(); j++)
			{
				int alpha = k2d::map_to_range(layers[i].at(j).output_value, -1, 1, 0, 255);
				node_sprites.at(node_index)->SetColor(k2d::Color(255, alpha));
				node_index++;
				...
				if (i < layers.size() - 1)
				{
					...
					for (size_t k = 0; k < layers[i + 1].size(); k++)
					{
						int alpha_w = k2d::map_to_range(layers[i].at(j).output_weights.at(k) * layers[i].at(j).output_value, -1, 1, 0, layer_alphas[i]);
						weight_sprites.at(weight_index)->SetColor(k2d::Color(255, alpha_w));
						weight_index++;
					}
				}
			}
